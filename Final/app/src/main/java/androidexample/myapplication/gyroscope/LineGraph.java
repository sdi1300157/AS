...
public class LineGraph {
	...
	public LineGraph()
	{
		...
		mRenderer.setXTitle("\nTime (ms)");
		mRenderer.setYTitle("   X-Y-Z axes values (rad/s)");
		mRenderer.setShowGrid(false);
		//mRenderer.setGridColor(Color.LTGRAY);
		mRenderer.setZoomEnabled(false);
		mRenderer.setYAxisMin(-15.0);
	...
	{
		series1.add(p.getX(), GyroGraph.xx);
		series2.add(p.getX(), GyroGraph.yy);
		series3.add(p.getX(), GyroGraph.zz);
